describe Moonshot::ASG do
  ...
  describe '#initialize' do
    ...
    context 'when CDB_CI_USER is not defined' do
      it 'uses LOGNAME for ssh_user' do
        ENV['LOGNAME'] = 'SemiCoolDude'
        ENV.delete('CDB_CI_USER')
        expect(subject.instance_variable_get(:@ssh_user))
          .to eq('SemiCoolDude')
      end
    end

    context 'when CDB_CI_USER is defined' do
      it 'uses CDB_CI_USER for ssh_user' do
        ENV['CDB_CI_USER'] = 'CoolDude'
        expect(subject.instance_variable_get(:@ssh_user))
          .to eq('CoolDude')
      end
    end
